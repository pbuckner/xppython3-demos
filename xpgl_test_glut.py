...
from OpenGL.GL import glClearColor, glClearDepth, glEnable, glViewport, \
    GL_TEXTURE_2D, glClear, GL_COLOR_BUFFER_BIT, GL_DEPTH_BUFFER_BIT, \
    glLoadIdentity, glOrtho, GL_BLEND, GL_TRUE, glDepthMask, glFlush
from OpenGL.GLUT import glutLeaveMainLoop, glutInit, \
    glutInitDisplayMode, glutTimerFunc, GLUT_RGBA, GLUT_DEPTH, \
    glutInitWindowSize, GLUT_STENCIL, glutInitWindowPosition, glutCreateWindow, \
    glutDisplayFunc, glutIdleFunc, glutKeyboardFunc, glutMainLoop, glutReshapeFunc
...
def keyPressed(key, _x, _y):
    # defined keys:
    #   'q'  kill everything with 0 error code
    #   'X'  kill with -1 error code
    #   'R'  Report opengl status to stdout
...
def ReSizeGLScene(width, height):
    # Our image will get distorted, enlarged/shrunk, as the window size and shape
    # changes -- lower left coordinate and upper right coordiant stay the same,
    # though no longer uniform.
...
def main():
    try:
        ...
        print("GLUT not properly installed")
        sys.exit(-1)
    ...
    glutCreateWindow("OpenGL tester")
    ...
    # mimic X-Plane counter
    def Loop(_ignore):
        xp.Counter += 1
        glutTimerFunc(29, Loop, 0)  # about 35 fps
    glutTimerFunc(29, Loop, 0)

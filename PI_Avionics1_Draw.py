...
# It differs from PI_Avionics2_Draw.py by setting a different
# custom_device id ("avnwx.avionics.device") and NOT being configured
# using EasyPython
# having a black bezel
...
class PythonInterface(EasyPython):
    ...
    def onStop(self: Self) -> None:
        # xp.log(f"Av1 {xpgl.Pattern=}, Texutres are: {xpgl.Textures}, {xpgl.XPGL_Texture_ID=}, {xpgl.Smooth_Lines=}")
    ...
    def create(self: Self) -> None:
        ...
        self.customAvionicsId = xp.createAvionicsEx(screenWidth=self.panel['screen'].width,
                                                    ...
                                                    refCon=self.panel,
                                                    deviceID="avnwx.avionics.device",
    ...
    def bezelDraw(self: Self, _r: float, _g: float, _b: float, refcon: Any) -> None:
        ...
        # (actually, this isn't required, because we've already drew a filled black
        # polygon for the bezel, fully under the screen. If we'd used a color other
        # than black for the polygon, the following polygon would be required)
    ...
    def draw(self: Self, refcon: Any) -> None:
        ...
        xpgl.drawMaskStart()
        # draw shape of mask vvvvvvvvv
        GL.glPushMatrix()
        GL.glTranslate(screen.width / 2, 190, 0)
        GL.glRotate(xp.getCycleNumber() % 360, 0, 0, 1)
        GL.glTranslate(-screen.width / 2, -190, 0)
        ...
        GL.glPushMatrix()
        GL.glTranslate(screen.width / 2, screen.height / 2, 0)
